...
| `lambda meta-prompt <intent> <Op[,Op…]> [--adjectives <file>] [--json]` | Compose a chain into ONE prompt for the stated intent. The chain sets the properties the prompt exhibits *simultaneously*, not steps run in order, and every clause traces to an operator field. `--adjectives` takes a JSON array of `{ op, adjective, reason }` substituting an operator's authored adjective for this intent; each substitution is labelled as the caller's in the brief and listed in `verification.callerSuppliedAdjectives`. See [operator-chain-as-prompt-policy.md](operator-chain-as-prompt-policy.md). |
