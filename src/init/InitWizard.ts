...
export class InitWizard {
  ...
  async run(): Promise<InitReport> {
    const detection = await new DetectHostsStep(this.registry, this.ctx, this.assets).run(undefined, this.io);
    ...
    const generate =
      this.writer === undefined
        ? new GenerateStep(this.ctx, this.assets, this.version)
        : new GenerateStep(this.ctx, this.assets, this.version, this.writer);
